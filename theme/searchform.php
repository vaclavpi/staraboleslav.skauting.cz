...
<form role="search" <?php echo $sablona_aria_label; ?> method="get" class="search-form" action="<?php echo esc_url( home_url( '/' ) ); ?>">
	<label for="<?php echo esc_attr( $sablona_unique_id ); ?>">
		<span class="screen-reader-text"><?php _e( 'Search for:', 'sablona' ); ?></span>
		<input type="search" id="<?php echo esc_attr( $sablona_unique_id ); ?>" class="search-field" placeholder="<?php echo esc_attr_x( 'Search &hellip;', 'placeholder', 'sablona' ); ?>" value="<?php echo get_search_query(); ?>" name="s" />
	</label>
	<input type="submit" class="search-submit" value="<?php echo esc_attr_x( 'Search', 'submit button', 'sablona' ); ?>" />
</form>
